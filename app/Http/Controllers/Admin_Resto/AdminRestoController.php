...
class AdminRestoController extends Controller
{
    ...
    public function stored(Request $request)
    {
        /* $request->validate([
            ...
            'no_telp' => 'required|min:5|max:13',
            'username' => 'required|min:5|max:20|unique:restorans',
            'map_resto' => 'required',
            'email' => 'required|email:dns|unique:restorans',
            'nama_pemilik' => 'required',
            'nohp_pemilik' => 'required|min:5|max:13|numeric',
            'provinsi' => 'required',
            'kabupatenkota' => 'required',
            'kapasitas' => 'required|numeric',
            'gambar' => 'required',
            'password' => 'required|min:5|max:13|numeric'
        ]); */
   ...
   public function logout()
    {
        ...
        return redirect()->intended(route('admin_resto.login'));
    }
